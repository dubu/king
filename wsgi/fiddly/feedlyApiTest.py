...
def makeFile(name ,seq ,category):
    ...
    if item['categories'][0]['label'].upper() in topicCatetory:
        print_format(", \"tags\":[ { \"category\":\"TOPIC\", \"tag\":\"TOPIC_%s\", \"name\":\"%s\", \"original_id\":\"tag_topic_blog\", \"abstract\":\"\", \"order_in_category\":4 } , { \"category\":\"TOPIC\", \"tag\":\"TOPIC_ALL\", \"name\":\"ALL\", \"original_id\":\"tag_topic_etc\", \"abstract\":\"\", \"order_in_category\":3 }]" %(item['categories'][0]['label'].upper() ,item['categories'][0]['label'].upper()) )
    else :
