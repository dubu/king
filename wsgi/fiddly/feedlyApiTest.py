...
def makeFile(name ,seq ,category):
    ...
    for item in content['items']:
        ...
        topicCatetory = ["SHORT","NEWS","MOVIE" ,"FUN"]
        if item['categories'][0]['label'].upper() in topicCatetory:
            ...
            print_format ("\"tags\": [\"TOPIC_ALL\"],")
            print_format ("\"mainTag\":\"TOPIC_ALL\",")
    ...
    if item['categories'][0]['label'].upper() in topicCatetory:
        print_format(", \"tags\":[ { \"category\":\"TOPIC\", \"tag\":\"TOPIC_ALL\", \"name\":\"ETC_ALL\", \"original_id\":\"tag_topic_all\", \"abstract\":\"\", \"order_in_category\":1 }]")
    else :
...
ff.write("{\"format\":\"iosched-json-v1\",\"data_files\":[\"enter_v%d.json\",\"it_v%d.json\",\"life_v%d.json\",\"media_v%d.json\",\"short_v%d.json\",\"alram_v%d.json\",\"movie_v%d.json\"]}" %(g_seq,g_seq,g_seq,g_seq,g_seq,g_seq,g_seq))
ff.close()
